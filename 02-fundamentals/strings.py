...
def cesky_cas_do_db_casu():
    datum = "12.10.2020"
    print(f"{datum[6:]}-{datum[3:5]}-{datum[:2]}")


def transform(souslovi, syntax):
    identifikator = "\0"
    if syntax == 'Python':
        souslovi = souslovi.lower()
        identifikator = souslovi.replace(' ', '_')
    elif syntax == 'JS':
        slovo = souslovi.split()
        identifikator = slovo[0].lower()
        for slovo in slovo[1:]:
            identifikator += slovo.capitalize()
    print(identifikator)
...
def generuj_hesla(pocet_osob):
    ...
    for _ in range(pocet_osob):
        # Generování hesla s požadovaným formátem
        ...
        # Složení hesla
        heslo = f"{velka_pismena}{mala_pismena}{spec_znak}{cislice}"
...
transform("To je proměnná v Pythonu", "Python")
transform("To je proměnná v Pythonu", "JS")

hesla = generuj_hesla(5)
for i, heslo in enumerate(hesla, start=1):
    print(f"Heslo pro osobu {i}: {heslo}")

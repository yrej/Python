...
# a)
hundreds = []
for cislo in range(1, 2001):
    if cislo % 200 == 0:
        hundreds.append(cislo)
print(hundreds)
# b)
import random
import string
ascii_seznam = ''.join(random.choice(string.ascii_uppercase) for _ in range(50))
ascii_seznam = list(ascii_seznam)
print(ascii_seznam) #kontrolni výpis
# c)
del hundreds[0:3]
del hundreds[-3:]
print(hundreds) #kontrolni výpis
# d)
unique = []
for znak in ascii_seznam:
    if ascii_seznam.count(znak) == 1:
        unique.append(znak)
print(unique)

# e)
ascii_seznam = ascii_seznam[:len(hundreds)]
combine = list(zip(hundreds, ascii_seznam))
print(combine)
...
print(f'\n*************************************\nCvičení 3\n*************************************')
# a)
persons.extend((("Pepa", 25, "muž"), ("Franta", 36, "muž"), ("Lenka", 42, "žena"), ("Eva", 19, "žena")))
# b)
women = [x[0] for x in filter(lambda x: 'žena' in x[2], persons)]

for i in range(len(women)):
    print("\033[4m", women[i], "\033[0m")

# c)
print("CSV struktura i-lidí")
print("---------------------")
ipeople = [person for person in persons if 'i' in person[0].lower()]
ipeople.sort(key=lambda person: person[1], reverse=True)
csv_data = "index;Jméno;Věk;Pohlaví\n"
